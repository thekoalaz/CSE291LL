...
void DiffuseEnvMap::_readMap()
{
    ...
    for (int x = 0; x < _width; x++)
    {
        ...
        {
            _setPixelR(x, y, _envMap._getPixelR(x, y));
            _setPixelG(x, y, _envMap._getPixelG(x, y));
            _setPixelB(x, y, _envMap._getPixelB(x, y));
        }
    }
        ...
        for (int j = 0; j < _height; j++){
            ...
            for (int k = 0; k < _width; k+=256){
                ...
                for (int l = 0; l < _height; l+=256){
                    ...
                    _setPixelR(i, j,  _getPixelR(i,j) + R*cosAng*sin(theta) * 2 * M_PI*M_PI / (_width*_height) * (256*256));
                    _setPixelG(i, j,  _getPixelG(i,j) + G*cosAng*sin(theta) * 2 * M_PI*M_PI / (_width*_height) * (256*256));
                    _setPixelB(i, j,  _getPixelB(i,j) + B*cosAng*sin(theta) * 2 * M_PI*M_PI / (_width*_height) * (256*256));
                }
            }
        }
    }
